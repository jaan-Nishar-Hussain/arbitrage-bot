import { BN, ZERO, ONE, PRECISION } from "@/utils/bn";
...
export class SimulatorService {
  ...
  static simulateSimpleArbitrage(
    ...
    try {
      ...
      const priceImpactA = this.calculatePriceImpact(
        amountIn,
        amountB,
        reservesA[0],
        reservesA[1]
      );
      const priceImpactB = this.calculatePriceImpact(
        amountB,
        amountOut,
        reservesB[0],
        reservesB[1]
      ...
      const profitPercent =
        amountIn > ZERO ? (Number(netProfit) / Number(amountIn)) * 100 : 0;
  ...
  static findOptimalAmount(
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        const result = this.simulateSimpleArbitrage(
          mid,
          reservesA,
          reservesB,
          gasEstimate
