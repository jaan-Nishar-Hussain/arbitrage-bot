...
export async function opportunitiesRoutes(
  ...
  fastify.get<{
    ...
    "/opportunities",
    async (
      request: FastifyRequest<{ Querystring: OpportunitiesQuery }>,
      reply: FastifyReply
    ) => {
      try {
        const {
    ...
    async (
      ...
      try {
        ...
        return {
          ...
          tokenPath: opportunity.tokenPath
            ? JSON.parse(opportunity.tokenPath)
            : null,
    ...
    async (
      ...
      try {
        ...
        const opportunities = await prisma.opportunity.findMany({
          ...
          orderBy: [{ profitPercent: "desc" }, { netProfit: "desc" }],
          take: limitNum,
        });
